...
class FinishPage {
  constructor(_pageManager, _session, _dataSender, _pageConfig, _language) {
    this.pageManager = _pageManager
    this.session = _session
    this.dataSender = _dataSender
    this.pageConfig = _pageConfig
  ...
  storeParticipantData() {
    for (const element of this.pageConfig.questionnaire) {
      this.session.participant.name.push(element.name)
      const responseValue =
        $("#" + element.name).val() ||
        $("input[name='" + element.name + "__response']:checked").val()
      this.session.participant.response.push(responseValue)
    }
  }

  sendResults() {
    const data = this.dataSender.send(this.session)
    if (!data) {
      this.errorDiv.text(
        "An error occurred while sending your data to the server! Please contact the experimenter."
      )
    }
    clearInterval(this.interval)
    return data
  ...
  render(_parent) {
    ...
    for (const element of this.pageConfig.questionnaire) {
      if (element.type === "text") {
        ...
        const options = element.response
          .map((el, i) => {
            const id_ = element.name + "_id_" + i
            return (
              '<input type="radio" name="' +
              element.name +
              '__response" id="' +
