...
Portions Copyright (C) Pieter Wolfert and contributors 2021.
...
function shuffle(a) {
  for (let i = a.length - 1; i > 0; i--) {
    ...
    ;[a[i], a[j]] = [a[j], a[i]] // Swapping elements using destructuring assignment
  }
  return a
}
...
class VideoPC {
  constructor(
    ...
    let i = 0
    for (const item in this.pageConfig.stimuli) {
      const key = this.pageConfig.stimuli[item][0]
      const stimuli = this.pageConfig.stimuli[item][1]
      const atVal = this.pageConfig.stimuli[item][2]

      let stimulus
      if (key === "at") {
        stimulus = newStimulus(i, key, stimuli, atVal)
      } else {
        stimulus = newStimulus(i, key, stimuli)
      }
      this.conditions.push(stimulus)
      i++
    }
  ...
  btnCallbackCondition = function (_index) {
  ...
  renderCanvas = function (_parentId) {
    ...
    var xLetters =
      $("#refCanvas").width() +
      ($("#spaceForScale").width() + canvasContext.measureText("1 ").width) /
        2.0
    ...
    $.post(
      ...
      (data = {
        ...
        interaction: JSON.stringify(
          $.extend(true, {}, this.interactionTracker)
        ),
  ...
  checkNumConditions = function (_conditions) {
    if (_conditions.length > 8) {
      this.errorHandler.sendError(
        "Number of conditions must be no more than 6."
      )
      return false
    }
    return true
...
function newStimulus(i, key, video, atVal) {
  const stimulus = new Stimulus(key, video)
